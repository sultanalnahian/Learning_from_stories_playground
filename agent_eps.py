...
task_command_list = ["disturb your team mates while they are doing works", "make fun of others", "wear the game gears", "avoid joining the meeting", "leave the room after the meeting"]
...
def play(phrase_mode, agent, gg_premodel, path, max_step=50, nb_episodes=30, verbose=True):
    ...
    policy_list = [] 
    task_specific_command_list = []
    altruistic_command_list = []
    for no_episode in range(nb_episodes):
        ...
        if no_episode%25 == 0:
            task_specific_command_list.append(no_task_command/len(task_command_list))
            altruistic_command_list.append(no_altruistic_command/len(altruism_command_list))
...
for phrase_mode in phrase_modes:
    ...
    tr_avg_scores, _, task_command_no_list, altruistic_command_no_list = play(phrase_mode, agent, gg_model, game_path, max_step=step_size, nb_episodes=2000, verbose=False)
...
plt.plot(all_task_specific_command_list[0], label="task")
plt.plot(all_altruistic_command_list[0], label="altruism")
...
file1 = open('result/test_phrase_2_quest_6_update_fr_2000.txt', 'w') 
