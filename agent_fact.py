...
task_command_list = ["disturb your team mates while they are doing works", "make fun of others", "wear the game gears", "join the meeting", "leave the room after the meeting"]
...
def play(phrase_mode, agent, gg_premodel, path, max_step=50, nb_episodes=30, verbose=True):
    ...
    for no_episode in range(nb_episodes):
        ...
        if no_episode%alt_interval == 0:
            task_specific_command_list.append((total_no_task_command/alt_interval)/len(task_command_list))
            altruistic_command_list.append((total_no_altruistic_command/alt_interval)/len(altruism_command_list))
            total_no_task_command = 0
            total_no_altruistic_command = 0
...
x_plot = [a*10 for a in range(int(nb_episodes/10)+1)]
...
plt.title("command type - original game phrase")
plt.plot(x_plot, all_task_specific_command_list[3], label="task")
plt.plot(x_plot, all_altruistic_command_list[3], label="altruism")
...
pickle.dump(all_task_specific_command_list, open( "result/task_ratio/task.p", "wb" ) )
pickle.dump(all_altruistic_command_list, open( "result/task_ratio/altruistic.p", "wb" ) )


print("task: ", all_task_specific_command_list)
print("altruism: ", all_altruistic_command_list)
file1 = open('result/test_all_phrases_quest_8_facts_5000.txt', 'w') 
